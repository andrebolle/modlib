...
func main() {
	...
	// Program
	vs := utils.ReadShader("SpecInstancing.vs.glsl")
	fs := utils.ReadShader("SpecInstancing.fs.glsl")
	...
	for !window.ShouldClose() {
		...
		gl.Uniform1f(gl.GetUniformLocation(program, gl.Str("uTime\x00")), float32(time))
		...
		gl.DrawElementsInstanced(gl.TRIANGLES, int32(len(*indices)), gl.UNSIGNED_INT, gl.PtrOffset(0), 2000)
		window.SwapBuffers()
