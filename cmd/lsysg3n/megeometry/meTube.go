...
func NewRibbon(paths [][]math32.Vector3, close bool) *geometry.Geometry {
	...
	*/
	...
	for i <= min && p < len(ls)-1 {
		...
		indices.Append(uint32(i+1), uint32(i+t), uint32(i))
		indices.Append(uint32(i+t), uint32(i+1), uint32(i+t+1))
		i++
		if i == min {
			if close {
				// Andre was here
				...
				indices.Append(uint32(is[p]), uint32(i+t), uint32(i))
				indices.Append(uint32(i+t), uint32(is[p]), uint32(is[p]+t))
			}
			p++
			if p == len(ls)-1 {
				break
	...
	// Andre was here
	for hello := 0; hello < len(normals); hello++ {
		normals[hello] = -normals[hello]
	}
...
// MyNewTube MyNewTube
func MyNewTube(path []math32.Vector3, radius float32, radialSegments int, close bool) *geometry.Geometry {
	l := len(path)
