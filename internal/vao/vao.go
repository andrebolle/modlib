...
// NewVAO NewVAO
func NewVAO(program uint32, floats *[]float32, attrSize int32) *VAO {
	vao := new(VAO)
	vao.VertexCount = int32(len(*floats) / int(attrSize))
	vao.Program = program
	...
	gl.VertexAttribPointer(0, attrSize, gl.FLOAT, false, 0, gl.PtrOffset(0))
